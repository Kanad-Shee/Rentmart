    ...
    >
      <div className="flex flex-col xl:flex-row">
        ...
        <div className="flex-1 p-6 md:p-8">
          ...
          <div className="mt-8 grid gap-4 border-t border-[#edf1ee] pt-6 text-sm text-[#5c5f60] md:grid-cols-1">
            <div>
...
export function OwnerRentalRequestsContent() {
  ...
  const grouped = useMemo(
    () => ({
      pending: bookings.filter((booking) => booking.status === "PENDING_OWNER_APPROVAL"),
  ...
  async function handleApprove(bookingId: string) {
    ...
    try {
      ...
      setFeedback("Booking approved. The renter can now complete Razorpay checkout.");
    } catch (error) {
      setActionError(error instanceof ApiError ? error.message : "Unable to approve this booking.");
    }
  }
  ...
  async function handleReject(bookingId: string) {
    ...
    if (reason.length < 5) {
      setActionError("Add a short rejection reason before declining the booking.");
      return;
    }
    ...
    try {
      ...
      setActionError(error instanceof ApiError ? error.message : "Unable to reject this booking.");
    }
  }
  ...
  async function handleStart(bookingId: string) {
    ...
    try {
      ...
      setActionError(error instanceof ApiError ? error.message : "Unable to start this booking.");
    }
  }
  ...
  async function handleComplete(bookingId: string) {
    ...
    try {
      ...
      setFeedback("Booking completed. Admin payout and deposit refund are now waiting for manual settlement.");
    } catch (error) {
      setActionError(error instanceof ApiError ? error.message : "Unable to complete this booking.");
    }
  }
  ...
  async function handleDispute(bookingId: string) {
    ...
    try {
      ...
      setActionError(error instanceof ApiError ? error.message : "Unable to dispute this booking.");
    }
  }
  ...
  return (
    <section className="space-y-16">
      <div>
        ...
        <p className="mt-3 max-w-3xl text-base leading-8 text-[#5c5f60]">
          Review renter requests, track renter payment, and manage each rental until admin settles payout manually after completion.
        </p>
      </div>
                      ...
                      <div className="flex items-start gap-3 rounded-xl border border-[#f5deb3] bg-[#fffaf0] p-4">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-[#9a6700]">
                            {getDeadlineLabel(booking.renterPaymentDeadlineAt) ?? "Payment window active"}
                          </p>
                          <p className="mt-1 text-xs leading-6 text-[#7c5a00]">
                            These dates stay blocked while the renter completes payment for this booking.
                          </p>
                        </div>
                      </div>
                    }
                    ...
                    actions={
                      <>
                        <button
                          ...
                          disabled={completeMutation.isPending || disputeMutation.isPending}
                          className="rounded-[4px] bg-[#1b4332] px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#274e3d] disabled:cursor-not-allowed disabled:opacity-70"
                        >
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full border-collapse">
                    <thead className="bg-[#f8faf7]">
                      <tr>
                        {["Item", "Renter", "Dates", "Total", "Status"].map((heading) => (
                          <th
                            key={heading}
                    ...
                    <tbody className="divide-y divide-[#edf1ee]">
                      {grouped.history.map((booking, index) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-5">
                            <span className={`inline-flex rounded-full px-3 py-1 text-sm font-medium ${getStatusTone(booking.status)}`}>
                              {getStatusLabel(booking.status)}
                            </span>
                          </td>
                        </motion.tr>
