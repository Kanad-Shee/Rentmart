import { ArrowUpRight, GlobeIcon, GridIcon, ShieldIcon } from 'lucide-react';
...
function CircleIcon({ label }: { label: string }) {
  return (
    <span className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-border bg-muted text-[10px] font-semibold uppercase tracking-[0.2em] text-primary">
      {label}
    </span>
  );
}
      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-16 lg:grid-cols-[1.2fr_0.8fr_0.8fr_1fr] lg:px-8">
        <div className="max-w-sm">
          <Link
            prefetch
            href="/"
            className="text-base font-brand font-black tracking-[0.24em] text-primary">
            RENTMART
          </Link>
          <p className="mt-4 text-sm font-display font-semibold text-muted-foreground">
          ...
          <div className="mt-6 flex items-center gap-4 text-muted-foreground">
            ...
            <GlobeIcon className="size-5 text-emerald-500" />
