  ...
  return (
    <div className='relative overflow-hidden rounded-lg border border-[#c1c8c2] bg-white'>
      <div className='relative h-28 w-full'>
        <Image src={photo.url} alt='Review photo' fill className='object-cover' unoptimized />
      </div>
      {onRemove ? (
        <button
          type='button'
...
function NewPhotoChip({
  ...
  return (
    <div className='relative overflow-hidden rounded-lg border border-[#c1c8c2] bg-white'>
      <div className='relative h-28 w-full'>
        <Image src={preview.url} alt='New review upload' fill className='object-cover' unoptimized />
      </div>
      <button
        type='button'
        onClick={onRemove}
...
export function ProductReviewsSection({ product }: { product: EquipmentDetails }) {
  ...
  const [form, setForm] = useState(buildFormFromReview(product.viewerReviewState.review));
  ...
  useEffect(() => {
    ...
    setRetainedPhotoIds(product.viewerReviewState.review?.images.map((image) => image.id) ?? []);
  ...
  const retainedExistingPhotos = useMemo(
    () =>
      (existingReview?.images ?? []).filter((image) => retainedPhotoIds.includes(image.id)),
    [existingReview?.images, retainedPhotoIds],
  );
  const totalSelectedPhotos = retainedExistingPhotos.length + newPhotos.length;
  const isMutating = createReviewMutation.isPending || updateReviewMutation.isPending;
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setErrorMessage(
        error instanceof ApiError ? error.message : "Unable to save your review right now.",
      );
    }
  }
    ...
    <section className='bg-[#f3f4f1] py-16'>
      <div className='mx-auto max-w-[1440px] px-4 sm:px-6 lg:px-8'>
        ...
        <div className='mt-8 flex justify-center'>
          ...
          >
            View All {product.reviewCount} Review{product.reviewCount === 1 ? "" : "s"}
          </button>
        </div>
      </div>
            ...
            <div className='border-b border-[#c1c8c2] px-6 py-4 sm:px-8'>
              ...
              <DialogHeader className='pr-12'>
                <DialogTitle className='text-2xl text-primary'>
                  {existingReview ? "Update your experience" : "Rate your experience"}
                </DialogTitle>
                <DialogDescription className='text-sm text-[#5c5f60]'>
                  Share how the equipment performed and how the rental experience felt.
                </DialogDescription>
              </DialogHeader>
            </div>
              ...
              <div className='space-y-6 px-6 py-6 sm:px-8'>
                <div className='flex items-center gap-4 rounded-lg border border-[#c1c8c2]/50 bg-[#f3f4f1] p-3'>
                  ...
                  <div>
                    ...
                    <p className='text-lg font-semibold text-primary'>{product.title}</p>
                  </div>
                </div>
                ...
                <div>
                  <div className='flex items-center justify-between gap-3'>
                    ...
                    <span className='text-xs text-[#717973]'>Optional, up to 5</span>
                  </div>
